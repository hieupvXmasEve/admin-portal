...
| **M3** | M | ⛔ **SPLIT 2026-05-19** into M3a + M3b per validation report. See `current-story-pack-M3.md` (superseded) + `m3-repair-memo.md`. | — | — |
| **M3a** | M | Test-send button (`NotificationTemplateController.php:160`) → outbox emit. Single callsite, same module, no Contract decision needed. Proves outbox-emit pattern before fan-out. | S2 ✅ + C1 ✅ | LOW-MEDIUM risk. **DONE 2026-05-19 (local). Pending staging.** |
| **H1** | H | Handler hygiene precursor for M3b: empty-string guard + caller-sanitize doc-comment on `HandleOutboxEventAction::buildRenderedEmail`. From M3a review P2 backlog. | M3a swarm-DONE locally | LOW risk; 3 file ops; behavior-preserving. |
| **M3b** | M | 6 Finance reminder callsites in 4 Actions → outbox emit. F3 locked to Option A: Finance injects `PublishDomainEventAction` directly (matches 6 established cross-module callers). Documents the exception in `docs/rules/contracts.md`. | H1 + M3a green-on-staging | HIGH risk; cross-module + production reminder traffic. |
