...
const isBalanceFocused = computed(() => props.focus?.type === 'payment' && props.focus.id === props.cards.balance.unapplied_payment_id);
const isDngFocused = computed(() => props.focus?.type === 'dng' && props.cards.dng.request?.id === props.focus?.id);
const isInstallmentFocused = computed(() => props.focus?.type === 'installment' && props.cards.installments.next?.id === props.focus?.id);
const isExceptionFocused = computed(() => props.focus?.type === 'dng' && props.cards.exception.dng_request_id === props.focus?.id && props.cards.dng.request?.id !== props.focus?.id);
...
<template>
    <div class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
        <Card data-focus-section="balance" :class="cn(isBalanceFocused && focusRing)">
        ...
        <Card data-focus-section="dng" :class="cn(isDngFocused && focusRing)">
            <CardHeader class="pb-1">
                <CardTitle class="text-xs font-medium">DNG cần xử lý</CardTitle>
            </CardHeader>
            <CardContent class="space-y-2">
                <template v-if="cards.dng.has_active && cards.dng.request">
                    ...
                    <Button v-if="actions.can_cancel_dng && ['pending', 'pushed_to_dng'].includes(cards.dng.request.status)" size="sm" variant="destructive" @click="emit('cancelDng', cards.dng.request.id)"> Hủy DNG </Button>
                </template>
                <p v-else class="text-muted-foreground text-sm">Không có DNG.</p>
            </CardContent>
        </Card>

        <Card data-focus-section="installments" :class="cn(isInstallmentFocused && focusRing)">
            <CardHeader class="pb-1">
            ...
            <CardContent class="space-y-2">
                <template v-if="cards.installments.total > 0">
                    ...
                    <div v-if="cards.installments.next" class="flex flex-wrap items-center gap-2">
                        <Badge :variant="cards.installments.next.has_push_error ? 'destructive' : 'secondary'"> Kỳ {{ cards.installments.next.installment_no }} · {{ cards.installments.next.due_date ?? '—' }} </Badge>
                        <Button
        ...
        <Card data-focus-section="exception" :class="cn(isExceptionFocused && focusRing)">
            <CardHeader class="pb-1">
            ...
            <CardContent class="space-y-2">
                <template v-if="cards.exception.needs_review">
                    ...
                    <Button v-if="actions.can_cancel_dng && cards.exception.dng_request_id" size="sm" variant="outline" @click="emit('review', cards.exception.dng_request_id)"> Review </Button>
                </template>
