...
import { StudentActionType, type ActionTypeOption, type Campus, type CourseRegistrationPreview, type EgcChargePreview, type EgcDeferBlockOption, type Semester, type StoreStudentActionForm } from '@/types/student-action';
...
watch(
    ...
    () => {
        ...
        form.egc_defer_from_block_number = form.action_type === StudentActionType.ACADEMIC_DEFER && isEgcStudent.value ? 1 : null;
            ...
            <form class="space-y-4" @submit.prevent="handleSubmit">
                ...
                <template v-if="selectedActionType === StudentActionType.ACADEMIC_DEFER">
                    ...
                    <div v-if="isEgcStudent" class="space-y-2">
                        ...
                        <Select :model-value="form.egc_defer_from_block_number ? String(form.egc_defer_from_block_number) : undefined" @update:model-value="(value) => (form.egc_defer_from_block_number = value ? Number(value) : null)">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="space-y-2">
                            ...
                            <Select v-model="form.to_campus_id">
                                ...
                                <SelectContent>
                                    <SelectItem v-for="campus in actionOptions.campuses.filter((c) => c.id !== form.from_campus_id)" :key="campus.id" :value="campus.id">
                                        {{ campus.name }}
                                    </SelectItem>
                                </SelectContent>
                            </Select>
                    ...
                    <div class="space-y-2">
                        ...
                        <Select :model-value="form.decision_id ? String(form.decision_id) : 'none'" @update:model-value="(value) => (form.decision_id = value === 'none' ? null : Number(value))">
                            ...
                            <SelectContent>
                                ...
                                <SelectItem v-for="decision in actionOptions.studentDecisions ?? []" :key="decision.id" :value="String(decision.id)"> {{ decision.decision_number }} - {{ decision.decision_name }} </SelectItem>
                            </SelectContent>
                        </Select>
