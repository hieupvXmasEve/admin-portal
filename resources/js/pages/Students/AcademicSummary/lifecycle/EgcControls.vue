...
const usesPlacementTest = computed(() => !placementForm.has_ielts || Number(placementForm.ielts_score) < placementOptions.value.ieltsScoreThreshold);
                ...
                <DialogContent class="max-h-[90vh] max-w-lg overflow-y-auto">
                    ...
                    <form class="space-y-4" @submit.prevent="handleIeltsSubmit">
                        ...
                        <div class="space-y-2">
                            ...
                            <FileUpload v-model="ieltsUploadedFiles" context="ielts_certificate" :upload-immediately="true" accept=".pdf,.jpg,.jpeg,.png" label="Drop the IELTS certificate scan here or click to upload" />
                    ...
                    <form class="space-y-4" @submit.prevent="handleTransitionSubmit">
                        <div class="space-y-2">
                            ...
                            <Select v-model="transitionForm.ielts_certificate_id">
                                ...
                                <SelectContent>
                                    <SelectItem v-for="option in ieltsCertificateOptions" :key="option.value" :value="option.value" :disabled="option.belowThreshold">
                                        {{ option.label }}
                                    </SelectItem>
                                </SelectContent>
                            </Select>
                            <p v-if="ieltsCertificateOptions.length === 0" class="text-muted-foreground text-xs">No IELTS certificates recorded yet — record one first.</p>
