...
const completionRate = (stats: NonNullable<SurveyData['completionStats']>): number => (stats.total > 0 ? Math.round((stats.completed / stats.total) * 100) : 0);
        ...
        <template v-if="!surveyData?.surveyTarget">
            <div class="py-12 text-center">
                ...
                <div class="mt-5 flex flex-col items-center gap-2">
                    ...
                    <p v-if="!surveyForms || surveyForms.length === 0" class="text-muted-foreground text-xs">No survey forms available. Please create a survey form first.</p>
                </div>
            </div>
        </template>
                ...
                <Card>
                    <CardHeader>
                        <div class="flex items-start justify-between gap-2">
                            ...
                            <Badge :variant="surveyStatusVariant(surveyData.surveyTarget.status)" class="shrink-0 capitalize">
                    ...
                    <CardContent class="space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <p class="text-muted-foreground text-xs font-medium tracking-wide uppercase">Start Date</p>
                            ...
                            <div v-if="surveyData.surveyTarget.end_at">
                                <p class="text-muted-foreground text-xs font-medium tracking-wide uppercase">End Date</p>
                                <p class="mt-0.5 font-semibold">
                    ...
                    <CardContent class="space-y-5">
                        <div>
                            ...
                            <div class="mt-2 h-2 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700">
                                <div class="h-2 rounded-full transition-all" :class="completionBarColor(completionRate(surveyData.completionStats))" :style="{ width: `${completionRate(surveyData.completionStats)}%` }"></div>
                            </div>
                            <p class="text-muted-foreground mt-1 text-xs">{{ completionRate(surveyData.completionStats) }}% completion rate</p>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4 text-center">
                            <div class="rounded-lg border p-3">
                                ...
                                <p class="text-muted-foreground mt-0.5 text-xs font-medium tracking-wide uppercase">Completed</p>
                            </div>
                            <div class="rounded-lg border p-3">
                                <p class="text-2xl font-bold text-orange-500">{{ surveyData.completionStats.pending }}</p>
                                <p class="text-muted-foreground mt-0.5 text-xs font-medium tracking-wide uppercase">Pending</p>
                            </div>
                        </div>
                    </CardContent>
                </Card>
