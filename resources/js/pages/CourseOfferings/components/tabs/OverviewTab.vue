    ...
    <div class="space-y-6">
            ...
            <Card class="lg:col-span-2">
                ...
                <CardContent class="space-y-5">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <p class="text-muted-foreground text-xs font-medium tracking-wide uppercase">Course Code</p>
                            <p class="mt-0.5 text-lg font-semibold">{{ courseOffering.course_code }}</p>
                        </div>
                        <div v-if="courseOffering.section_code">
                            <p class="text-muted-foreground text-xs font-medium tracking-wide uppercase">Section</p>
                            <p class="mt-0.5 text-lg font-semibold">{{ courseOffering.section_code }}</p>
                        </div>
                    </div>

                    <div>
                        <p class="text-muted-foreground text-xs font-medium tracking-wide uppercase">Course Title</p>
                        <p class="mt-0.5 text-lg font-semibold">{{ courseOffering.course_title }}</p>
                    </div>

                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <p class="text-muted-foreground text-xs font-medium tracking-wide uppercase">Delivery Mode</p>
                            <p class="mt-0.5 font-semibold">{{ getDeliveryModeLabel(courseOffering.delivery_mode) }}</p>
                        </div>
                        <div v-if="courseOffering.location">
                            <p class="text-muted-foreground text-xs font-medium tracking-wide uppercase">Location</p>
                    ...
                    <div v-if="courseOffering.notes">
                        <p class="text-muted-foreground text-xs font-medium tracking-wide uppercase">Notes</p>
                ...
                <CardContent class="space-y-5">
                    <div>
                        ...
                        <div class="mt-2 h-2 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700">
                            <div class="h-2 rounded-full transition-all" :class="getEnrollmentColor()" :style="{ width: `${enrollmentPercentage}%` }"></div>
                        </div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <p class="text-muted-foreground text-xs font-medium tracking-wide uppercase">Waitlist</p>
                            <p class="mt-0.5 font-semibold">{{ courseOffering.current_waitlist }}/{{ courseOffering.waitlist_capacity }}</p>
                        </div>
                        <div>
                            <p class="text-muted-foreground text-xs font-medium tracking-wide uppercase">Available</p>
        ...
        <div class="grid gap-6 lg:grid-cols-2">
            ...
            <Card>
                ...
                <CardContent class="space-y-5">
                    <div>
                        <p class="text-muted-foreground text-xs font-medium tracking-wide uppercase">Semester</p>
                    ...
                    <div>
                        <p class="text-muted-foreground text-xs font-medium tracking-wide uppercase">Lecturer</p>
                        <p class="mt-0.5 font-semibold">{{ courseOffering.lecture?.display_name || 'Not assigned' }}</p>
                    </div>

                    <div v-if="courseOffering.curriculum_unit">
                        <p class="text-muted-foreground text-xs font-medium tracking-wide uppercase">Curriculum Unit</p>
                        <!-- /units/:id has no named route yet — kept as literal path -->
                        <Link :href="`/units/${courseOffering.curriculum_unit.unit.id}`" class="mt-0.5 flex items-center gap-1.5 text-sm font-semibold text-green-500 hover:underline">
                    ...
                    <div>
                        <p class="text-muted-foreground text-xs font-medium tracking-wide uppercase">Syllabus</p>
                        <div v-if="courseOffering.syllabus_template" class="mt-0.5 space-y-1.5">
                            <Link :href="curriculumRoutes.syllabusTemplates.show(courseOffering.syllabus_template.id)" class="flex items-center gap-1.5 font-semibold hover:underline">
            ...
            <Card>
                ...
                <CardContent class="space-y-5">
                    <div>
                        <p class="text-muted-foreground text-xs font-medium tracking-wide uppercase">Registration Period</p>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <p class="text-muted-foreground text-xs font-medium tracking-wide uppercase">Drop Deadline</p>
                            <p class="mt-0.5 text-sm">{{ formatDate(courseOffering.drop_deadline) }}</p>
                        </div>
                        <div>
                            <p class="text-muted-foreground text-xs font-medium tracking-wide uppercase">Withdrawal Deadline</p>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <p class="text-muted-foreground text-xs font-medium tracking-wide uppercase">Schedule Days</p>
                            <p class="mt-0.5 text-sm font-medium">
                                {{ courseOffering.schedule_days?.join(', ') ?? 'Not set' }}
                            </p>
                        </div>
                        <div>
                            <p class="text-muted-foreground text-xs font-medium tracking-wide uppercase">Schedule Time</p>
                            <p class="mt-0.5 text-sm font-medium">{{ courseOffering.schedule_time_start ?? 'N/A' }} — {{ courseOffering.schedule_time_end ?? 'N/A' }}</p>
                        </div>
                    </div>
                </CardContent>
            </Card>
                ...
                <div class="flex flex-wrap gap-2">
                    <Link v-for="sibling in siblingOfferings" :key="sibling.id" :href="route('course-offerings.show', sibling.id)">
                        <Button variant="outline" size="sm">
                            ...
                            <Badge variant="secondary" class="ml-1.5 px-1.5 py-0 text-[11px]"> {{ sibling.current_enrollment }}/{{ sibling.max_capacity }} </Badge>
                        </Button>
                    </Link>
                </div>
            </CardContent>
