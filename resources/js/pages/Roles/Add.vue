                        ...
                        <div>
                            ...
                            <div class="max-h-32 overflow-y-auto">
                                <div class="flex flex-wrap gap-1">
                                    <span v-for="permissionId in form.permissions" :key="permissionId" class="bg-muted text-muted-foreground inline-flex items-center rounded border px-2 py-1 text-xs font-medium">
                                        {{ permissionGroups.find((group) => group.permissions.some((permission) => permission.id === permissionId))?.permissions.find((permission) => permission.id === permissionId)?.display_name }}
                                    </span>
                                </div>
                            </div>
                        </div>
                ...
                <div class="space-y-6">
                    <div v-for="permissionGroup in permissionGroups" :key="permissionGroup.module" class="space-y-3">
                        ...
                        <div class="bg-muted/50 flex items-center gap-3 rounded-lg p-3">
                            <Checkbox :model-value="isModulePermissionSelected(permissionGroup)" :indeterminate="isModulePermissionPartiallySelected(permissionGroup)" @update:model-value="toggleModulePermissionSelection(permissionGroup)" />
                            <div class="flex-1">
                        ...
                        <div v-if="permissionGroup.permissions && permissionGroup.permissions.length > 0" class="ml-6 space-y-2">
                            <div v-for="permission in permissionGroup.permissions" :key="permission.id" class="hover:bg-muted/30 flex items-center gap-3 rounded p-2">
                                <Checkbox :model-value="isPermissionSelected(permission.id)" @update:model-value="togglePermissionSelection(permission.id)" />
                                <div class="flex-1">
