...
const addComponentDetail = (componentIndex: number) => {
    ...
    const next = current.map((component, index) => (index === componentIndex ? { ...component, details: [...(component.details || []), { name: '', weight: null }] } : component));
    form.setFieldValue('assessment_components', next);
};
const removeComponentDetail = (componentIndex: number, detailIndex: number) => {
    const current = (form.values.assessment_components as any[]) || [];
    const next = current.map((component, index) => (index === componentIndex ? { ...component, details: (component.details || []).filter((_: unknown, di: number) => di !== detailIndex) } : component));
    form.setFieldValue('assessment_components', next);
};
