...
const mappedAnswers = computed(() => {
    ...
    return selectedAssignment.value.response.answers.map((ans: any) => ({
        ...
        selected_options: ans.selected_options ? ans.selected_options.map((opt: any) => ({ id: opt.id, text: opt.label, value: opt.value })) : [],
    }));
});
...
const responseColumns: ColumnDef<StudentAssignment>[] = [
    {
        ...
        cell: ({ row }) => {
            ...
            return h('div', { class: 'flex flex-col' }, [h('div', { class: 'font-medium' }, a.student?.full_name || 'Unknown'), h('div', { class: 'text-xs text-muted-foreground' }, a.student?.student_id || '---')]);
        },
    },
    {
        header: 'Completed At',
    ...
    },
    {
        id: 'actions',
        header: 'Actions',
        cell: ({ row }) => h(Button, { variant: 'outline', size: 'sm', disabled: !row.original.response_id, onClick: () => openResponseDetail(row.original) }, () => 'View Detail'),
    },
];
</script>
...
<template>
    ...
    <div v-if="navigation.total > 1" class="bg-muted/50 sticky top-16 z-20 flex items-center justify-between border-b px-4 py-1.5 text-sm">
        ...
        <span class="text-muted-foreground tabular-nums"> {{ navigation.current_index ?? '?' }} / {{ navigation.total }} </span>
    ...
    <div class="px-4 py-8">
        ...
        <Deferred data="sections">
            <template #fallback>
                <div v-if="activeTab === 'summary' || activeTab === 'question'" class="animate-pulse space-y-4">
                                    ...
                                    <CardContent class="space-y-3">
                                        ...
                                        <div class="space-y-1.5">
                                            <div class="flex items-center gap-2">
                                                ...
                                                <span class="w-10 text-right text-xs text-green-600 tabular-nums">{{ section.stats!.positive_percent }}%</span>
                                            </div>
                                            <div class="flex items-center gap-2">
                                                ...
                                                <div class="h-2 flex-1 overflow-hidden rounded-full bg-amber-100">
                                                    <div class="h-full bg-amber-400 transition-all" :style="{ width: 100 - section.stats!.positive_percent - section.stats!.negative_percent + '%' }"></div>
                                                </div>
                                                <span class="text-muted-foreground w-10 text-right text-xs tabular-nums"> {{ 100 - section.stats!.positive_percent - section.stats!.negative_percent }}% </span>
                                            </div>
                                            <div class="flex items-center gap-2">
                                                ...
                                                <span class="w-10 text-right text-xs text-red-500 tabular-nums">{{ section.stats!.negative_percent }}%</span>
                                            </div>
                                        </div>
                                    </CardContent>
                                </Card>
                            ...
                            <Card v-for="q in section.questions" :key="q.id" class="overflow-hidden">
                                ...
                                <CardContent class="pt-6">
                                    ...
                                    <div v-else-if="['single_choice', 'likert'].includes(q.type)" class="flex items-center justify-center">
                                        <DoughnutChart v-if="q.data.options?.length > 0" :data="getChoiceChartData(q.data.options)" :options="doughnutChartOptions" :plugins="doughnutPlugins" height="250px" width="100%" />
                            ...
                            <Card class="border-t-primary border-t-4">
                                ...
                                <CardContent class="pt-8">
                                    ...
                                    <div v-else-if="['single_choice', 'likert'].includes(currentQuestion.type)" class="flex items-center justify-center">
                                        <DoughnutChart v-if="currentQuestion.data.options?.length > 0" :data="getChoiceChartData(currentQuestion.data.options)" :options="doughnutChartOptions" :plugins="doughnutPlugins" height="320px" width="100%" />
        ...
        <div v-if="activeTab === 'responses'">
            <Deferred data="responses">
                ...
                <template #default="{ reloading }">
                    <div class="space-y-4" :class="{ 'pointer-events-none opacity-60': reloading }">
                        <!-- Filters -->
                        <div class="bg-card space-y-4 rounded-lg border p-4">
                            <div class="flex flex-wrap items-center gap-4">
                                <div class="min-w-[250px] flex-1">
                                    <DebouncedInput placeholder="Search students..." :model-value="respFilters.search" @update:model-value="respHandleSearch" />
                                </div>
