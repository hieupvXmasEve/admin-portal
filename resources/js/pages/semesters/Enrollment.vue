...
// Table columns for suggested courses
const suggestedCoursesColumns: ColumnDef<SuggestedCourse>[] = [
    {
        id: 'select',
        header: ({ table }) =>
            h('input', {
                ...
                onChange: (event: Event) => {
                    ...
                    if (target.checked) {
                        suggestedCourses.value.forEach((course) => selectedCourses.value.add(course.unit.id));
                    } else {
                        selectedCourses.value.clear();
                    }
        ...
        cell: ({ row }) =>
            h('input', {
                type: 'checkbox',
                checked: selectedCourses.value.has(row.original.unit.id),
                onChange: (event: Event) => {
                    const target = event.target as HTMLInputElement;
                    if (target.checked) {
                        selectedCourses.value.add(row.original.unit.id);
                    } else {
                        selectedCourses.value.delete(row.original.unit.id);
                    }
                },
            }),
    },
