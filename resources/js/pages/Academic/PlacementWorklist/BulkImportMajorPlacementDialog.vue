...
const skipReasonLabels: Record<string, string> = {
    ...
    application_score_missing: 'No qualifying IELTS application',
    below_threshold_excluded: 'Below 5.5 threshold',
    score_out_of_range: 'Score outside plausible IELTS range',
};
...
const reasonText = (row: ImportRow): string => {
    if (row.error) return row.error;
    if (!row.skip_reason) return '-';
    const label = skipReasonLabels[row.skip_reason] ?? row.skip_reason;
    return row.skip_reason === 'below_threshold_excluded' && row.overall_score != null ? `${label} (score ${row.overall_score})` : label;
};
...
const handlePreview = async (): Promise<void> => {
    ...
    const result = await submit(route('students.placement-worklist.bulk-import-major.preview'), false);
...
const handleExecute = async (): Promise<void> => {
    ...
    const result = await submit(route('students.placement-worklist.bulk-import-major.execute'), true);
...
<template>
    <Dialog v-model:open="isOpen" @update:open="(open) => !open && close()">
        <DialogContent class="max-h-[90vh] !max-w-4xl overflow-y-auto">
            <DialogHeader>
                <DialogTitle>Bulk Import Major Placement</DialogTitle>
                <DialogDescription>
                    Upload a CSV with a `Student ID` column. The IELTS score is looked up from each student's latest
                    qualifying application — preview before placing students.
                </DialogDescription>
            </DialogHeader>

            <div class="space-y-2">
                <Label for="bulk-major-file">CSV / Excel File *</Label>
                <Input id="bulk-major-file" type="file" accept=".csv,.txt,.xlsx,.xls" @change="onFileChange" />
                    ...
                    <table class="w-full text-sm">
                        <thead class="bg-muted/50 sticky top-0">
                            <tr class="border-b">
                                ...
                                <th class="px-3 py-2 text-left font-medium">IELTS Score</th>
                        ...
                        <tbody>
                            <tr v-for="row in previewResult.rows" :key="row.row_number" class="border-b" :class="row.status === 'skip' ? 'text-muted-foreground' : ''">
                                ...
                                <td class="px-3 py-2">{{ row.overall_score ?? '-' }}</td>
                                <td class="px-3 py-2">
                                    <Badge :class="rowBadgeClass(row)">{{ row.status.toUpperCase() }}</Badge>
                                </td>
                                <td class="px-3 py-2">{{ reasonText(row) }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
