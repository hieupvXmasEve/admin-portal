...
const applyFilters = () => {
    const cleanFilters = Object.fromEntries(Object.entries(filters.value).filter(([, value]) => value !== '' && value !== 'all' && value !== null));
...
const columns: ColumnDef<Club>[] = [
    ...
    },
    {
        ...
        cell: ({ row }) => {
            ...
            return h('div', { class: 'flex items-center gap-2' }, [h(Building, { class: 'h-4 w-4 text-gray-500' }), h('span', campus?.name || 'No campus')]);
        },
    },
    {
        header: 'President',
        ...
        cell: ({ row }) => {
            ...
            return h('div', { class: 'text-sm' }, [h('div', { class: 'font-medium' }, president.student.full_name), h('div', { class: 'text-gray-500' }, president.student.student_id)]);
        },
    },
    {
        header: 'Members',
        ...
        cell: ({ row }) => {
            ...
            return h('div', { class: 'flex items-center gap-2' }, [
                ...
                h('div', { class: 'text-sm' }, [h('div', { class: 'font-medium' }, `${activeCount} active`), pendingCount > 0 && h('div', { class: 'text-gray-500' }, `${pendingCount} pending`)]),
            ]);
        },
    },
    {
    ...
    },
    {
        ...
        cell: ({ row }) => {
            ...
            return h('div', { class: 'flex items-center gap-2 text-sm' }, [h(Calendar, { class: 'h-4 w-4 text-gray-500' }), new Date(date).toLocaleDateString()]);
        },
    },
    {
        id: 'actions',
    ...
    <div class="mt-6 space-y-4">
        <div class="flex flex-wrap items-center gap-4 rounded-lg border p-4">
            ...
            <div class="min-w-[300px] flex-1">
                <DebouncedInput v-model="filters.search" @debounced="handleSearch" placeholder="Search clubs by name or description..." />
            </div>
