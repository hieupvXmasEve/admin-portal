...
const initialValues: TuitionPlanFormData = {
    curriculum_version_id: undefined as any,
    intake_semester_id: undefined as any,
    total_amount: 0,
    currency: 'VND',
    is_active: true,
    terms: [],
...
const onSubmit = handleSubmit(async (formValues, actions) => {
    ...
    router.post(route('tuition-plans.store'), submitData, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
        ...
        onError: (serverErrors) => {
            isSubmitting.value = false;
            toast.error('Failed to create tuition plan. Please check the form for errors.');
...
<template>
    <Head title="Create Tuition Plan" />

    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold tracking-tight">Create Tuition Plan</h1>
                <p class="text-muted-foreground">Define a new tuition plan for a curriculum version and intake period</p>
            </div>
        ...
        <form @submit.prevent="onSubmit">
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>Enter the basic information for the tuition plan</CardDescription>
                </CardHeader>
                            ...
                            <Select v-bind="componentField">
                                ...
                                <SelectContent>
                                    <SelectItem
                                        ...
                                        :value="cv.id"
                                    >
                                        {{ cv.program.name }} - {{ cv.version_code }}
                                    </SelectItem>
                                </SelectContent>
                            </Select>
                            <FormMessage />
