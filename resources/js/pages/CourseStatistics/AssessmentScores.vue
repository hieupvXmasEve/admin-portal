...
const getScoreDisplay = (score: Score): string => {
    ...
    return score.percentage_score.toFixed(1);
};
            ...
            <CardContent>
                <div class="flex flex-wrap gap-4 text-sm">
                    <div class="flex items-center gap-2">
                        <Badge variant="success">85.0</Badge>
                        <span>Excellent (≥80%)</span>
                    </div>
                    <div class="flex items-center gap-2">
                        <Badge variant="warning">70.0</Badge>
                        <span>Pass (60-79%)</span>
                    </div>
                    <div class="flex items-center gap-2">
                        <Badge variant="destructive">50.0</Badge>
                        <span>Fail (&lt;60%)</span>
                    </div>
                    <div class="flex items-center gap-2">
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow>
                                ...
                                <TableHead v-for="detail in assessment_details" :key="detail.id" class="min-w-[100px] text-center">
                                    <div class="space-y-1 text-xs">
                                        <div class="font-bold">{{ detail.component_name }}</div>
                                        <div class="text-muted-foreground">{{ detail.detail_name }}</div>
                                        <Badge variant="outline" class="text-xs">{{ getComponentTypeLabel(detail.component_type) }}</Badge>
                                        <div class="text-muted-foreground">Weight: {{ detail.component_weight }}%</div>
                                    </div>
                                </TableHead>
                        ...
                        <TableBody>
                            <TableRow v-for="student in filteredScoresGrid" :key="student.student_id">
                                ...
                                <TableCell v-for="score in student.scores" :key="score.component_detail_id" class="text-center">
                                    <div class="flex flex-col items-center gap-1">
                                        <Badge :variant="getScoreBadgeVariant(score)">
                                            {{ getScoreDisplay(score) }}
                                        </Badge>
                                        <div v-if="score.percentage_score !== null" class="text-muted-foreground text-xs">
                                            {{ score.letter_grade || '-' }}
                                        </div>
                                        <div v-if="score.is_late && score.percentage_score !== null" class="text-xs text-orange-600">Late</div>
                                    </div>
                                </TableCell>
                                ...
                                <TableCell class="sticky right-0 z-10 bg-white text-center dark:bg-gray-950">
                                    <div class="space-y-1">
                                        <Badge v-if="student.total_percentage !== null" :variant="getTotalBadgeVariant(student.total_percentage)" class="text-base font-bold"> {{ student.total_percentage.toFixed(1) }}% </Badge>
