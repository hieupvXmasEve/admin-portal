...
const filteredCurriculumVersions = computed(() => {
    ...
    return versions.filter(
        (version) =>
            ...
            (version.effective_from_semester?.name && version.effective_from_semester.name.toLowerCase().includes(curriculumFilters.value.search.toLowerCase())),
    );
});
...
const curriculumVersionColumns: ColumnDef<CurriculumVersion>[] = [
    {
        ...
        cell: ({ row }) => {
            ...
            return h('div', { class: 'space-y-1' }, [
                ...
                version.effective_from_semester ? h('div', { class: 'text-xs text-muted-foreground' }, `From: ${version.effective_from_semester.name}`) : null,
            ]);
        },
    },
    {
...
const curriculumPaginationData = computed(() => ({
    ...
    next_page_url: curriculumFilters.value.page < Math.ceil(paginatedCurriculumVersions.value.total / 10) ? `?page=${curriculumFilters.value.page + 1}` : null,
    links: [] as any[],
}));
...
<template>
    ...
    <Card>
        <CardHeader class="pb-4">
                        ...
                        <Form :validation-schema="formSchema" @submit="onSubmit" class="space-y-4">
                            ...
                            <FormField v-slot="{ componentField }" name="notes">
                                <FormItem>
                                    ...
                                    <FormControl>
                                        <Textarea v-bind="componentField" placeholder="Enter any additional notes..." rows="3" :maxlength="ValidationRules.curriculumVersion.notes.maxLength" />
                                    </FormControl>
                                    <FormMessage />
                                </FormItem>
                            </FormField>
            ...
            <!-- Search Filter -->
            <div class="mt-4">
                <DebouncedInput v-model="curriculumFilters.search" @debounced="handleCurriculumSearch" placeholder="Search curriculum versions..." class="max-w-sm" />
            </div>
        </CardHeader>
            ...
            <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                    ...
                    <span v-if="versionToDelete?.curriculum_units_count && versionToDelete.curriculum_units_count > 0" class="text-destructive">
                        <br /><br />
                        <strong>Warning:</strong> This version has {{ versionToDelete.curriculum_units_count }} curriculum units that will also be affected.
                    </span>
                </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
                <AlertDialogCancel :disabled="isDeleting" class="cursor-pointer">Cancel</AlertDialogCancel>
                <AlertDialogAction @click="confirmDelete" :disabled="isDeleting" class="bg-destructive hover:bg-destructive/90 cursor-pointer text-white">
