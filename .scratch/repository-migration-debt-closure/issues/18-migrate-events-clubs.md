...
- [ ] Staff UI and student API surfaces use the owner boundary and current frontend/API conventions. Admin Event endpoints still need FormRequests and ApiResponse envelopes.
- [ ] Student identity and notification interactions use approved contracts rather than shared business-model coupling. Registration, check-in, club membership, and notification audiences use contracts; manual participant workflows still query the shared Student model.
