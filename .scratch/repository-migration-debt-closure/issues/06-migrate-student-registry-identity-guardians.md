...
- 2026-07-23: Partial staff profile-update cutover. The staff `StudentService::updateStudent` path (used by `Academic\\Http\\Web\\StudentController::update`) now sends supported profile/contact fields to Student Registry. Email updates use the Registry `StudentIdentityWriter`; its Registry transaction updates the Student contact email and delegates the linked account email to Identity through `StudentAccessWriter`. `high_school_graduation_year`, `entrance_exam_score`, and `admission_notes` remain explicit transitional direct fields because their Admissions ownership is not yet characterized. No route or response envelope changed, so no student/guardian portal source update was needed. Data impact: none — no backfill, cleanup, or data command was run. Rollback is a revert of the implementation commit; the endpoint, Registry writer, and transaction-rollback tests are the observability signals. The issue remains `ready-for-agent` with all acceptance criteria open: Guardian runtime callers, remaining legacy writers/reads, and full compatibility evidence remain.
