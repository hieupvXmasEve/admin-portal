...
- [x] Notification V2 outbox, messages, deliveries, preferences, templates, SMTP resolution, retries, and monitoring have supported owner paths.
- [x] Domain producers use the shared publisher after commit and do not write delivery state directly.
- [x] Campus scope, permissions, channel selection, idempotency, retries, and audit evidence remain intact.
- [x] Staff and portal notification contracts remain compatible while legacy history has an explicit retained or approved migration disposition.
- [x] Legacy email/notification services and controllers retire only after scheduled, queued, and template-driven callers cut over.
...
- 2026-07-23: Completed. PHP test memory is now 1 GB. After correcting the notification-template test fixtures to use the request campus and clear permission/rate-limit cache state after direct fixture writes, the issue-scope suite passed: 107 tests / 363 assertions (`tests/Feature/Notification` plus the Finance reminder and Academic resit-cancellation producers). The full suite's `StudentAcademicSummaryScoresSchemeDisplayTest` still returns `409` rather than `200`; it belongs to the separate Metropolia Academic Summary work and does not block this issue.
