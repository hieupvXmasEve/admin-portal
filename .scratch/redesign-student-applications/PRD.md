...
- **Completing missing legacy data:** legacy rows with gaps are completed via a read-only **export → CRM supplies the data**, not an in-app update path (which would collide with freeze-at-approval ADR-0003 / revoke-after-activity ADR-0002). See issue `11-export-applications-for-crm-completion`. Open question deferred there: how completed data flows back for the ~229 legacy rows already `enrolled` (frozen).
