...
- [ ] Every supported fee type and next-installment push uses guarded reservation with exact line/installment targets, target fingerprint, settlement version, and deterministic provider identity.
- [ ] No DNG request can exceed canonical remaining collectible or proceed from an invalid Settlement Position.
- [ ] Provider calls occur outside database locks and ambiguous outcomes remain held for reconciliation.
- [ ] Arbitrary amount override is absent from validation, API payloads, UI, and domain actions.
- [ ] Legacy collection mode and unguarded single/batch creation methods have no callers and are removed.
- [ ] Selected-fee and pay-all behavior remain distinct and preserve exact fee-type breakdowns.
- [ ] Focused tests cover each fee family, installments, retry/idempotency, stale targets, invalid positions, and unknown provider outcomes.
