...
it('omits Canvas-reserved syllabus templates from the create page', function () {
    ...
    actingAs($this->user)
        ...
        ->assertInertia(fn ($page) => $page
            ...
            ->has('syllabusTemplates', 1)
            ->where('syllabusTemplates.0.id', $reusable->id));
});
...
it('keeps the current mapped syllabus visible on edit but omits another mapped syllabus', function () {
    ...
    actingAs($this->user)
        ...
        ->assertInertia(fn ($page) => $page
            ...
            ->has('syllabusTemplates', 2)
            ->where('syllabusTemplates.0.id', $current->id)
            ->where('syllabusTemplates.1.id', $reusable->id));
});
