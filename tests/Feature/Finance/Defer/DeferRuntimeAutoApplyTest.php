...
it('settles a historical auto-safe case via finance:defer-backfill --apply-money', function () {
    [$student, $deferCase] = autoApplyLegacyCase($this->semester, $this->campus, $this->program, $this->user, 'BF-MONEY', DeferCase::POLICY_FORFEIT);
    autoApplyRegistration($student->id, $this->semester->id, 'defer');
    $charge = autoApplyObligation($student->id, $this->semester->id, 45_000_000);
    autoApplyPay($student, $charge, 45_000_000, $this->user);

    $this->artisan('finance:defer-backfill', [
