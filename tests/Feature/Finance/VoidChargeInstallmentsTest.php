...
it('backfills live installments on already-voided charges', function () {
    [, , $charge] = makeVoidScenario();

    // Charge is already void (legacy data) but still has a live pending installment.
    $charge->update(['status' => FinanceCharge::STATUS_VOID, 'voided_at' => now()]);
    $installment = FinanceChargeInstallment::create([
        'finance_charge_id' => $charge->id,
        'installment_no' => 1,
        'amount' => 10_000_000,
    ...
    $this->artisan('finance:backfill-voided-charge-installments --dry-run')
        ->assertSuccessful();
    expect($installment->fresh()->status)->toBe(FinanceChargeInstallment::STATUS_PENDING);

    // Real run cancels it.
    $this->artisan('finance:backfill-voided-charge-installments')
        ->assertSuccessful();
    expect($installment->fresh()->status)->toBe(FinanceChargeInstallment::STATUS_CANCELLED);

    // Invariant INV-13 is now clean.
    $live = FinanceChargeInstallment::query()
        ->whereIn('status', [
            FinanceChargeInstallment::STATUS_PENDING,
            FinanceChargeInstallment::STATUS_AWAITING_PAYMENT,
        ])
        ->whereHas('charge', fn ($q) => $q->where('status', FinanceCharge::STATUS_VOID))
        ->count();
    expect($live)->toBe(0);
});
