...
2. 筛选找到需要处理的学生或课程。
3. 记录重修登记。系统按当前价格目录自动计算重修学费；若该课程尚未配置价格，系统会拦截并要求先完成配置。
...
2. 按学期、课程或学生筛选。
3. 创建补考批次并选择参加的学生。每条成绩记录允许的补考次数以课程规定为准（默认 1 次）；在 syllabus 中调高次数即可重新开放补考，无需其他改动。
4. 考试结束后录入结果以结束该批次。若学生已有考试时段但未参加，点击 **Vắng thi**（缺考）并确认——补考次数将被消耗，原成绩保持不变。

**注意事项**

- **缺考（No-show）**不同于**取消（Cancel）**：缺考指已有考试时段而未参加，费用不予退还；取消发生在考试之前，费用处理方式由下方选择决定。
- 取消**已付款**的补考登记时，可选择两种结果之一：**费用没收**（已收费用保留为收入）或**留作后续使用**（费用转为未用余额，用于该学生后续费用）。两种情况均不退款至学生账户。
