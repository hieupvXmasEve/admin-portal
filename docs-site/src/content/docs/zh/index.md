---
title: Portal 使用指南
description: 面向学务运营教职员工的 Portal 使用指南，按照系统实际菜单编写。
source:
...
- 菜单路径写作 **菜单分组 → 项目 → 子项目**。
- 界面上显示的名称目前是英文，因此本指南保留原文并在旁边加上中文注释。例如：**Programs（培养方案）**。
- 每个界面按四部分说明：*用途 · 谁可以访问 · 操作步骤 · 注意事项*。
- 如果指南中提到的某个项目不在你的菜单上，说明该账号尚未获得该项目的访问权限。请联系管理员。

## 致文档维护者

每个页面在文件头部声明 `source`——该页面所描述的源代码文件列表。当这些源文件发生变化时，`scripts/check-docs-freshness.sh` 会在同一个 pull request 中提示更新对应页面。
