...
4. **Semester**는 학생의 입학 학기가 기본값이며 변경할 수 있습니다. 분류 방식을 선택합니다: **EGC (placement test)** — **English Level**을 선택; **Major (IELTS)** — IELTS 점수를 입력합니다(스캔 누락 표시 후 추후 보완 가능). 기준 미달 점수는 EGC로 배정된다는 경고가 표시됩니다.
