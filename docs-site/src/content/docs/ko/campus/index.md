...
**Campus**는 수업 외의 공간과 활동을 관리합니다. 강의실과 행사가 해당합니다.

Gold 교환 스토어와 동아리는 별도 그룹인 **Store & Clubs**로 이동했습니다.
...
**절차.** **Campus → Room Management → Rooms**로 이동해 강의실을 추가하거나 수정합니다.
...
1. **Campus → Room Management → Find Available Rooms**로 이동합니다.
...
1. **Campus → Room Management → Pending Approvals**로 이동합니다.
...
**절차.** **Campus → Room Management → Room Usage Calendar**로 이동합니다. **Today**를 누르면 오늘로 돌아옵니다.
...
**절차.** **Campus → Events → List**로 이동해 행사를 만들거나 기존 행사를 엽니다.
...
| 재시험 시간대 편성 | Room Usage Calendar → Find Available Rooms → Exam Resit Schedule |
