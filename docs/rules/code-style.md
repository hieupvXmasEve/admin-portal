...
### 4.4 Formatting & Style

- Follow Prettier/ESLint output and `.editorconfig` (4-space indent).
- Use semicolons and single quotes in JS/TS/Vue.
- Keep import order handled by tooling (organize-imports).

### 4.5 Reka-UI Checkbox Binding

- Always bind checkbox via `:model-value` + `@update:model-value`.
