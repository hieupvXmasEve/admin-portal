...
## 4. Toast Notifications (vue-sonner)

### 4.1. Core Usage

- **Import**: `import { toast } from 'vue-sonner'`
- **Standard**: Always use `toast.success()` for successful mutations and `toast.error()` for failures.

### 4.2. Implementation Patterns

#### With Inertia `useForm`

When using `useForm` for data submission, trigger toasts within the `onSuccess` and `onError` callbacks.

```typescript
const form = useForm({ ... });

const submit = () => {
  form.post(route('tickets.assign'), {
    onSuccess: () => {
      toast.success('Ticket assigned successfully');
      form.reset('note');
    },
    onError: (errors) => {
      // Show first validation error if exists, else generic error
      const firstError = Object.values(errors)[0];
      toast.error(firstError || 'Failed to assign ticket');
    },
  });
};
```

#### With Manual `router` Calls

For simple actions like status updates using the `router` helper:
...
- Display short, actionable messages (e.g., "Reply posted", "Saved", "Assignment updated").
- Avoid redundant toasts for simple GET requests or navigation.
