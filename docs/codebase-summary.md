...
- AI module (`app/Modules/AI/`) now has provider settings, audit/evaluation foundations, metric catalog/query-plan validation, internal query-metrics/entity-search/profile-section tool execution, and the first staff copilot chat surface:
    ...
    - Staff copilot: `/ai/copilot`, `/ai/copilot/messages`, `/ai/copilot/runs/{run}/events`, `/ai/copilot/runs/{run}/cancel`, and `/ai/copilot/runs/{run}/retry` are gated by `view_ai_metrics`; message submission queues a durable run and assistant placeholder, the SSE endpoint replays normalized run events by cursor, rejects invalid cursors with `invalid_run_event_cursor` before execution, and executes the existing live/deterministic runner, cancellation is owner/campus checked, and retry reuses the original user message while creating a fresh assistant run. Active-run props expose `last_event_id` plus `replay_cursor`, and the Vue page reconnects from the latest applied cursor while de-duplicating event ids. Terminal run events include redacted `audit_evidence` for actor/campus scope, AI access layers, provider/model/runtime metadata, tool/source evidence, safe error reason, final answer linkage, cancellation, duration, and retry linkage. When a staff-owned provider setting is enabled and successfully tested, `LiveStaffCopilotAgent` uses Laravel AI SDK structured planner/final-answer agents, validates model-proposed tool calls, executes accepted reads through ToolDispatcher, persists provider usage/tool-call evidence, and falls back to deterministic `StaffCopilotAgentRunner` behavior when live mode is unavailable or fails safely.
    - Cross-module source reads for AI metrics, entity search, and profile sections go through shared contracts (`App\Shared\Contracts\Academic\AiAcademicMetricReader`, `App\Shared\Contracts\Academic\AiAcademicEntitySearchReader`, `App\Shared\Contracts\Academic\AiAcademicStudentProfileReader`, `App\Shared\Contracts\Finance\AiFinanceMetricReader`, `App\Shared\Contracts\Finance\AiFinanceStudentProfileReader`) and thin adapters in the owning Academic/Finance modules.
    - Runtime boundary: internal staff chat with provider-agnostic downstream SSE, fallback snapshot answer deltas, optional live provider planning/final synthesis, and read-only metric/entity/profile-section tools; no WebSocket/Reverb/Pusher dependency, MCP exposure, write/action mode, raw source-row exposure, raw profile exports, or student/lecturer portal behavior yet.
