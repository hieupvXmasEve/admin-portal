...
const { leaveChannel, leave, stopListening, listen } = useEcho(`orders.${orderId}`, 'OrderShipmentStatusUpdated', (e) => {
    console.log(e.order);
});
...
Echo.private(`App.Models.User.${userId}`).stopListeningForNotification(callback);
```
