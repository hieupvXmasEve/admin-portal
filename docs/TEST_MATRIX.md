...
| --------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- | ---- | ----------- | --- | -------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| S-001-lecturer-gpa-report                           | Staff can review active/default-semester Lecturer GPA by class-level evaluation average, link from Survey Results, and export to Excel         | no   | yes         | no  | no       | implemented | `tests/Feature/Lecture/LecturerGpaReportTest.php`; 3 tests/43 assertions passed; route check, targeted Pint, ESLint, Prettier, and `git diff --check` passed; repo-wide `vue-tsc` still fails on pre-existing typing drift outside touched Lecturer GPA files           |
| S-001-active-semester-teaching-hours-summary-export | Staff can review teaching-hours summary by active/default semester, optional date range, lecturer/course columns, and export the filtered view | yes  | yes         | no  | no       | implemented | `tests/Feature/Lecture/GetTeachingHoursActionTest.php`; 3 tests/18 assertions passed; targeted Pint, ESLint, Prettier, and touched-file `git diff --check` passed; repo-wide vue-tsc failed on pre-existing diagnostics                                                 |
...
| S-002-bulk-link-students-to-decision                | Staff can choose an action type, paste up to 100 student codes, preview matches, and bulk-link only existing unlinked action logs of that type | yes  | yes         | no  | no       | implemented | `tests/Unit/Academic/StudentCodeParserTest.php`, `tests/Feature/Academic/StudentDecisionBulkLinkTest.php`; 4 tests/27 assertions passed; route-list, targeted Pint, Prettier, ESLint, and `git diff --check` passed; repo-wide vue-tsc fails on pre-existing drift     |
