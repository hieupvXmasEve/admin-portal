...
### 36. `scholarship_definitions` & `student_scholarship_awards` (Học bổng)
...
### 37. `student_wallets` & `gold_transactions` (Ví điểm Gold)
...
### 38. `student_cash_wallets` & `wallet_transactions` (Ví tiền mặt)
...
### 39. `voucher_definitions` & `voucher_redemptions` (Voucher/Mã giảm giá)
...
### 40. `tuition_plans` & `tuition_plan_terms` (Kế hoạch học phí)
...
### 41. `billing_cycles` (Chu kỳ thanh toán)
...
### 42. `student_invoices`, `invoice_items`, `invoice_discounts` (Hóa đơn)
...
### 43. `forms` & `form_versions` (Biểu mẫu)
...
### 44. `form_targets` & `form_result_visibility` (Đối tượng và quyền xem kết quả)
...
### 45. `responses`, `answers`, `answer_options` (Phản hồi biểu mẫu)
...
### 46. `student_form_assignments` (Phân công biểu mẫu cho sinh viên)
...
### 47. `form_surveys` & `student_form_surveys` (Khảo sát đánh giá giảng viên)
...
### 48. `query_topics`, `queries_tickets`, `query_replies`, `query_assignments` (Hỗ trợ sinh viên)
...
### 49. `clubs` & `club_members` (Câu lạc bộ)
...
### 50. `events` & `event_participants` (Sự kiện)
...
### 51. `notifications` (Thông báo)
- **Lý do**: Lưu trữ các thông báo gửi đến người dùng.
- **Bảng liên quan**: `notifications`
- **Model**: `App\Models\Notification`

### 52. `email_configurations`, `email_templates`, `email_logs` (Cấu hình Email)
...
### 53. `user_email_preferences` (Tùy chọn email người dùng)
...
### 54. `upload_records` (Bản ghi upload)
...
### 55. `activity_log` (Nhật ký hoạt động)
