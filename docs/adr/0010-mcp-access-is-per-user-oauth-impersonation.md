...
A connector spike confirmed that ChatGPT needs no additional OAuth metadata
from the pinned build. Passport does not audience-bind tokens to the RFC 8707
`resource` parameter; this remains an accepted residual for compatible MCP
clients. Tightening it requires a separate decision.
