...
/**
 * Data migration: give existing campuses the scholarship-confirmation email
 * template. New campuses get it from the CampusObserver, but campuses created
 * before this type_key existed would otherwise have no row — and without one
 * the send falls back to the bare notification title and body.
 *
...
return new class extends Migration
{
    ...
    {
        // Only this type_key: the other templates predate this migration and
        // may carry admin edits.
        NotificationEmailTemplate::query()
            ->where('type_key', NotificationTemplateTypeKey::ScholarshipAdjustmentConfirmationRequested->value)
            ->delete();
    }
};
