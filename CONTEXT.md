...
> **Migration note (ADR-0026, ADR-0041):** the shared `app/Models/*` layout is *current state*, not target. Academic and Finance do **not** share money/lifecycle models, and the current `Student` model is a transitional God Model whose stable identity moves behind Student Registry. Cross-context consumers use neutral references and `app/Shared/Contracts/*`, not shared Eloquent business state.
...
| **Actor** _(Identity & Access)_ | The role in which an authenticated account uses Swinx — `student`, `guardian` (currently exposed as `parent`), or `lecturer`. An Actor is not a bounded context: authentication and access belong to Identity & Access, while the Actor's business profile belongs to its owning domain. _Avoid_: `Student`, `Parent`, or `Lecturer` modules organized around portal users. |
| **Bounded Context** | A business ownership boundary with its own language, rules, and source of truth. It need not map one-to-one to a physical Module while Swinx is migrating. |
| **Module** | A physical code boundary under `app/Modules/*` owning Actions, Queries, routes, and policies. A Bounded Context may first live as an internal namespace inside a Module; portal Actors do not define Modules. |
...
| **Cross-context Command Contract** | A synchronous request for an owning context to perform a state change when the caller cannot complete its use case without the result. The owner enforces its own invariants; the caller never writes the owner's models directly. |
| **Cross-context Read Projection** | A purpose-built, rebuildable view combining facts from multiple contexts for reporting, dashboards, or AI. It may be stale and is never the sole evidence for a hard business gate. |
...
| **Current Academic Period** _(Academic Catalog & Calendar)_ | The single institution-wide Academic Period Swinx resolves when staff ask for current-period metrics. If Swinx cannot resolve exactly one current period, Staff Copilot must ask for clarification before querying. _Avoid_: a separate current Semester per campus. |
...
| **Student Identity** _(Student Registry)_ | The stable identity of a Student across contexts: identifiers, display name, contact details, campus affiliation, and account linkage. Other contexts share only a Student Reference; Student Identity is not the source of truth for academic lifecycle or money behavior. _Avoid_: "Student as God Model". |
| **Student Reference** _(Shared Kernel)_ | The minimal stable reference used to identify a Student across bounded contexts. It carries identity needed for correlation, not Academic, Finance, Admissions, or Engagement state. |
| **Institution Campus** _(Institution & Organization)_ | A canonical organizational location identified by Swinx's own code, name, and address. Academic, Registry, Finance, and Facilities reference it but do not add provider-specific identifiers to it. |
| **Department** _(Institution & Organization)_ | An organizational unit responsible for people, operations, or academic offerings. Academic Catalog may reference the responsible Department, but it does not own the Department's identity or hierarchy. |
| **DNG Campus Mapping** _(Finance)_ | Finance-owned configuration mapping an Institution Campus to DNG's external campus code. It is integration metadata, not part of the Campus identity. |
| **Student–Guardian Relationship** _(Student Registry)_ | The durable relationship between a Student and every responsible Guardian, including relationship type and which Guardian is primary. It exists regardless of whether the Guardian has an email address or portal account. |
| **Guardian Access Grant** _(Identity & Access)_ | The revocable authorization allowing a Guardian Actor to access a Student through the portal at a defined access level. It requires an account but does not establish or replace the Student–Guardian Relationship. |
| **Faculty Member** _(Faculty Workforce)_ | A person employed or contracted to teach. Faculty Workforce owns their employee identifier, employment and contract state, academic rank, qualifications, expertise, teaching preferences, and pay terms; their login account and individual course assignments belong to other contexts. A Lecturer Actor is the access role through which a Faculty Member uses the lecturer portal. |
| **Teaching Eligibility** _(Faculty Workforce)_ | The Faculty Workforce-owned answer to whether a Faculty Member is professionally and contractually eligible to teach a Unit or type of course. It does not reserve workload or prove timetable availability. |
| **Instructor Assignment** _(Course Delivery & Assessment)_ | The assignment of an eligible Faculty Member to a Course Offering. Course Delivery owns the resulting teaching load, timetable, and conflict checks, combining Teaching Eligibility with its current offerings and assignments. |
| **Faculty Access Eligibility** _(Faculty Workforce)_ | The Workforce-owned signal describing whether a Faculty Member's employment and contract state permits lecturer access, with a reason. It informs Identity & Access but is not itself an account status or access grant. |
| **Lecturer Access Grant** _(Identity & Access)_ | The revocable authorization allowing a Lecturer Actor to use lecturer surfaces. Login and token refresh read this grant and Account Status, not Faculty Workforce tables; Workforce transitions may synchronously revoke it through an Identity contract. |
| **Academic Catalog & Calendar** | The Bounded Context owning Programs, Curriculum Versions, Units, syllabi, Academic Periods, and campus-specific period schedules. It defines what may be taught and when, not a Student's progress or a Course Offering's operations. |
| **Course Delivery & Assessment** | The Bounded Context owning Course Offerings, rosters, sessions, attendance, assessment evidence, exams, Instructor Assignments, and Course Results. |
| **Academic Progression & Lifecycle** | The Bounded Context owning Program Enrollments, Study Stages, Transcript Entries, GPA, Academic Standing, EGC progression, lifecycle actions, and graduation decisions. |
| **Migration Debt** _(Platform)_ | A supported runtime path that still violates an accepted ownership boundary or depends on a transitional compatibility pattern after a canonical replacement exists. Code is not Migration Debt merely because it is old or named `Service`; permanent components with current ownership remain valid. _Avoid_: "all old code", "legacy code" without naming the violated boundary or replacement. |
| **Academic Student Lifecycle** _(Academic Progression & Lifecycle)_ | The study state and rights of a Student through their Program Enrollments: registration, progression, defer/resume/dropout/return, eligibility, and graduation. Finance may consume it through events, contracts, read models, or snapshots, but must not infer it from internal Academic relationships. |
| **Program Enrollment** _(Academic Progression & Lifecycle)_ | The history-bearing relationship that places a Student in a Program, Curriculum Version, Intake Academic Period, and applicable academic lifecycle. Swinx currently permits at most one primary active Program Enrollment per Student; Program and Curriculum state do not belong to Student Identity. |
| **Account Status** _(Identity & Access)_ | The state controlling whether an account may authenticate and use Swinx, independently of any Student or Faculty business lifecycle. _Avoid_: deriving login eligibility directly from Program Enrollment Status or Study Stage. |
| **Program Enrollment Status** _(Academic Progression & Lifecycle)_ | The lifecycle state of one Program Enrollment, such as pending, active, deferred, withdrawn, or completed. It applies to the enrollment, not to Student Identity. _Avoid_: generic "Student status". |
| **Study Stage** _(Academic Progression & Lifecycle)_ | The Student's current learning stage within a Program Enrollment, such as Pre-Uni EGC, course, or major. It is distinct from Program Enrollment Status and Account Status. |
...
| **Course Offering Cockpit** _(Course Delivery & Assessment)_ | The single course-offering-centric operational console where academic staff manage one `CourseOffering`'s setup, roster, sessions, attendance (recording and readiness), scores, completion, surveys, and Canvas-linked constraints. The Course Offering detail page owns operations; Course Statistics and the Attendance page remain report-only surfaces. Not a parallel "Course Statistics management" flow. |
| **Canvas grade sync** _(Course Delivery & Assessment)_ | Pulling students' Canvas submission scores into a course offering's assessment grid, for all students or a selected subset. Canvas is the source of truth: applying a sync overwrites local component scores, and it is only available while the offering is not completed — afterwards grades change only through Recalculate. |
| **Sync preview** _(Course Delivery & Assessment)_ | The informational diff staff review before a Canvas grade sync or Recalculate applies: what changes per student and component, what cannot sync and why, and (for Recalculate) which students would be notified. A preview commits nothing — the apply step reads Canvas afresh. |
| **Readiness blocker** _(Course Delivery & Assessment)_ | A declared condition that prevents a course-offering lifecycle action (e.g. finalize) until resolved — such as sessions without recorded attendance, or a Canvas-mapped offering not yet synced. The Course Offering Cockpit renders blockers from the backend contract; the frontend never infers them. |
| **Academic Attendance Rate** _(Course Delivery & Assessment)_ | The primary attendance KPI for academic and BOD reporting: attended recorded sessions divided by attendance-applicable recorded sessions. Late attendance counts as attended; excused, official leave, medical leave, and not-recorded sessions are not penalizing absence evidence. |
| **Operational Presence Rate** _(Course Delivery & Assessment)_ | A secondary attendance operations KPI comparing present or late active roster students against expected active attendees for a class session or aggregate report. It highlights attendance-taking and roster compliance separately from the academic attendance KPI. |
| **Academic Period** _(Academic Catalog & Calendar)_ | The institution-wide named span used to organize teaching, progression, reporting, and financial correlation, currently represented as a Semester. A campus-specific calendar may override operating dates or registration windows when campuses genuinely differ, but it does not create a second identity for the Academic Period. _Avoid_: campus-owned duplicate Semesters. |
| **Campus Period Schedule** _(Academic Catalog & Calendar)_ | A campus-specific override of operating dates or registration windows within one institution-wide Academic Period. It never creates a second Academic Period identity. |
| **Selected Academic Period** _(Application State)_ | The period a user selects for navigation, filtering, or reporting. It is session/UI state, not the Current Academic Period or an Academic lifecycle fact. |
| **Space Availability** _(Facilities)_ | The Facilities-owned answer describing whether a Building or Room can satisfy a requested teaching or exam slot. Course Delivery owns the timetable intent; Facilities owns the physical reservation and conflicts. |
| **Course Result** _(Course Delivery & Assessment)_ | The finalized outcome of one Student's attempt in one Course Offering, derived from the offering's assessment and attendance evidence. Detailed scores and grading rules remain owned by Course Delivery & Assessment. |
| **Transcript Entry** _(Academic Progression & Lifecycle)_ | The record created from a finalized Course Result and used for transcript, GPA, Academic Standing, best-attempt, and graduation decisions. It does not expose or own the source gradebook's component scores. |
| **Course Completion** _(Course Delivery & Assessment / Academic Progression & Lifecycle)_ | The atomic transition that finalizes a Course Offering's Course Results and commits their Transcript Entries before the offering is considered completed. Notifications and other integrations happen only after that commit; Recalculate uses the same boundary for corrected results. |
| **Curriculum Module** _(Academic Catalog & Calendar)_ | A named grouping of Units inside a Curriculum Version (e.g. "Software 1") that receives its own aggregate grade and pass status. Distinct from **Module** (the code-architecture term above). |
| **Module grade** _(Academic Progression & Lifecycle)_ | The credit-weighted average of a Curriculum Module's graded (0-5) units, rounded once to a whole grade. It exists only once every graded unit has passed — no provisional average is shown. Pass/fail units are excluded from the average, but every unit — graded or pass/fail — must pass for the module to pass. Module grades never feed GPA — GPA stays unit-based. |
| **Best attempt** _(Academic Progression & Lifecycle)_ | When a Student has multiple Transcript Entries for the same Unit, the attempt with the highest grade represents that Unit in module aggregation; the Unit counts as passed if any attempt passed. |
| **EGC** _(Academic Progression & Lifecycle)_ | The Pre-Uni GC English-preparation phase represented by a Program Enrollment's Study Stage. The Academic Progression stream tracks a Student's journey into, through, and out of EGC. |
| **Student Action** _(Academic Progression & Lifecycle)_ | A formal event that changes a Program Enrollment's lifecycle or placement — academic defer, resume, dropout, campus transfer, admission deferral. Recorded in the action log. Some action types require an authorizing Decision. Not a generic "Student status change". |
| **Academic Progression** _(Academic Progression & Lifecycle)_ | A Student's English-level and course-stage journey through EGC, recorded as progression events: placement, English-level change, course-stage change, and an *IELTS recorded* event. The IELTS event is only a timeline marker that points to an **IELTS certificate** — the scored record lives separately, not inside the progression event. Distinct from Student Action (enrollment status, not level). |
| **Placement** _(Academic Progression & Lifecycle)_ | The first Academic Progression event for a Student entering EGC — their initial English level and course stage. Not a separate concept; it is progression's starting point. |
| **IELTS certificate** _(Academic Progression & Lifecycle)_ | A recorded IELTS result for a Student — overall band score and issue date — used to qualify a Student's transition from `intake_pre_uni_gc` to `intake_course`. The scored record itself, distinct from its supporting **IELTS scan**. _Avoid_: "IELTS record", "IELTS document". |
| **IELTS scan** _(Academic Progression & Lifecycle)_ | The uploaded document evidencing an **IELTS certificate**. When absent it is flagged "scan missing"; the `intake_course` transition requires it unless staff explicitly allow missing documents. Not the band score (that is the certificate). _Avoid_: "IELTS document". |
| **Decision** _(Academic Progression & Lifecycle)_ | A formal governance document (number, signer, issue/expiry date, scan) that covers one or more Students. It may authorize lifecycle events — a Student Action and/or an EGC progression transition (into `intake_pre_uni_gc` or `intake_course`) — or be purely informational (attached to Students without changing status). Some transitions require an authorizing Decision, which may be attached after the transition; a missing-decision report surfaces those still lacking one. Not a separate status track. |
...
| **Payable Line** _(Finance)_ | The smallest settlement unit against which cash, discounts, and credits are applied. All collectible totals for a charge, invoice, billing account, or collection request aggregate the derived positions of their payable lines. _Avoid_: deriving collectible debt from a gross fee row alone. |
| **Settlement Ledger** _(Finance)_ | The Finance-owned record of payable lines and their payment, discount, and credit applications used to derive what remains collectible. It is the permanent collection truth downstream of obligations and entitlements, not a temporary compatibility layer. _Avoid_: "legacy ledger", "old ledger". |
| **Settlement Position** _(Finance)_ | The authoritative derived money position of one or more payable lines, keeping gross, discount, matched cash, applied credit, remaining collectible, settlement state, and integrity distinct. Raw ledger evidence may be shown for audit, but every authoritative total comes from this position; inconsistency makes it invalid rather than silently normalized. |
| **Installment Plan** _(Finance)_ | The scheduled portions of a payable's collectible amount. Pending portions may be reconciled when settlement reductions change, but portions already committed to collection or paid are historical commitments and are never silently rewritten. |
| **Settlement Mutation Guard** _(Finance)_ | The short-lived payer-level concurrency boundary that serializes commands capable of changing or committing a Settlement Position. It prevents overlapping decisions during a transaction but is not a persistent hold on every payable belonging to the payer. |
| **Settlement Version** _(Finance)_ | A monotonically increasing revision of a Billing Account's settlement and collection state. Reservations and mutations use it to prove that the Settlement Position on which they acted is still current. |
| **Collection Reservation** _(Finance)_ | A local, auditable commitment of specific payable lines or installments and an amount before Finance calls an external collection provider. While pending, its collection hold protects only those linked targets; unrelated payables of the same Billing Account remain operable. |
| **Collection Request Cancellation** _(Finance)_ | The termination of an external payment request and release of its reservation or hold. It does not cancel, void, or otherwise change the underlying obligation, payable line, or source workflow. |
| **Finance-Pending Cancellation** _(Academic/Finance)_ | The non-terminal source state used while Finance resolves collection requests, paid cash, and obligation effects required by a cancellation. The source becomes cancelled only after Finance confirms completion; an unknown external outcome keeps it pending for review. |
| **Finance Cancellation Operation** _(Finance)_ | The durable, idempotent record of Finance work required to complete a source-requested obligation cancellation. It tracks collection resolution, settlement disposition, review evidence, and terminal completion independently of UI polling or request lifetime. |
| **DNG Active Slot** _(Finance)_ | The single unpaid collection slot for one Billing Account and one DNG fee type, aggregating the eligible due payable targets behind that type with an exact breakdown. Multiple unpaid requests of the same type never coexist; payment access may select this one type or combine all active types. |
| **Paid DNG Bridge** _(Finance)_ | The idempotent recording of externally confirmed DNG cash as a canonical Payment before Finance treats it as settled. Exact links allocate it to payable lines; without safe links the cash remains **Còn dư** for review rather than being ignored or guessed onto a fee. |
| **Unknown Collection Outcome** _(Finance)_ | A collection attempt for which Finance cannot yet prove whether the external provider accepted or rejected the request. It remains a collection hold until reconciliation establishes a terminal outcome. |
...
| **Đã thu** _(Finance)_ | The portion of a Student's real paid money that has been matched to current fee obligations. Discounts and applied credits reduce what remains collectible but are never included in this amount. |
| **Credit đã áp dụng** _(Finance)_ | The portion of an approved Finance credit that has been applied to payable lines. It reduces **Còn phải thu** but is not cash received and must remain separate from **Đã thu**. |
| **Còn phải thu** _(Finance)_ | The amount the school still needs to collect from the Student after valid discounts, matched cash, and applied credits reduce current fee obligations. Use over "balance" or "outstanding" in staff-facing student finance pages. |
...
| **Cần kiểm tra** _(Finance)_ | A staff-facing review state showing that finance evidence needs attention, such as unmatched cash or an invalid Settlement Position. It never blocks viewing and does not mean the Student necessarily owes more, but an integrity failure blocks affected payment or collection actions until resolved. |
...
| **Approve** _(Admissions)_ | The atomic decision to admit an Applicant, orchestrating creation of Student Identity, guardian relationships, account access, and Program Enrollment through their owning contexts. Not "convert", "admit", or "enroll". |
| **Reject** _(Admissions)_ | The decision that an Applicant will not become a Student. Not "decline" or "deny". |
| **Guardian** _(Admissions / Student Registry)_ | A parent or other responsible adult linked to an Applicant or Student. Admissions owns the Applicant relationship; approval preserves every Guardian in Student Registry whether or not they can use the portal. Use over "parent" when the relationship may not be a parent. |
...
| **Intake** _(Admissions)_ | The Academic Period an Applicant intends to start in, identified by its canonical code. With the Intended Program it determines the Curriculum Version the Student is enrolled under. |
