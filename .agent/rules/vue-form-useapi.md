...
Use these rules whenever a Vue form validates on the client with `vee-validate` + Zod and persists data through the `useApi`/`useApiRequest` composable stack instead of a full Inertia visit.
...
5. **Sync editing values**
    ...
    - On cancel, run `resetForm()` to clear dirty state.
...
- Decide whether to reset or keep the form. Use `resetForm()` after creates, or keep values when editing repeatedly.
...
- Inputs rely on `defineField`/`FormField` so errors and events are wired correctly.
