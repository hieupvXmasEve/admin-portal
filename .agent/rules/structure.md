...
- **app/**
    - **Actions/**: Business use-cases organized by domain (e.g., `app/Actions/Room/CreateRoomAction.php`)
    ...
    - **Services/**: Technical support and infrastructure services
...
- **Actions**: `Verb` + `Entity` + `Action` (e.g., `CreateRoomAction`, `BookRoomAction`)
- **Controllers**: PascalCase with `Controller` suffix (e.g., `UserController`)
- **Models**: PascalCase singular (e.g., `User`, `CourseOffering`)
- **Services**: PascalCase with `Service` suffix (e.g., `EmailService`, `StorageService`)
...
- Is reusable across Web, API, Jobs, Commands
- Does NOT return HTTP responses
- Does NOT know about controllers, requests, or views

### Rules for Actions

- One file = one use-case
- Naming: `Verb + Entity + Action`
- Located under `app/Actions/{Domain}/`
- Can depend on Services
- Can throw domain exceptions

### Service Layer (SUPPORTING ONLY)

### What is a Service?

A **Service** handles **technical or infrastructure logic** that may be reused.
...
- Services MUST NOT contain business workflows
- Services MUST NOT check roles or permissions
- Services MUST NOT orchestrate use-cases
