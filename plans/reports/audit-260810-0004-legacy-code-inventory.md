...
## Update 2026-08-16 (plan `260815-1320-close-remaining-legacy-shims-and-final-dead-cleanup`, Phases 3-5 — sweep CLOSED)

Verified via git log + code inspection (phases 3-5 were implemented and committed in a prior session; this update reconciles docs to that reality — no code changed in this pass):

- **Phase 3** (`586a52670`): `GoldTransaction` shim deleted. Reclaim guard + audit-trail queries moved into `GoldService::hasPendingEventTransaction()` / `getEventRewardAuditTrail()` — both confirmed present. `EventParticipationOperations` no longer imports the Merchandise model.
- **Phase 4** (`f24114c31`): `UploadRecord` shim deleted. `app/Shared/Contracts/Upload/UploadRecordReader.php` contract confirmed present (batch keyed-map shape, not id-list — enumeration-oracle guard intact).
- **Phase 5** (`f06b3ab6c`): `ApplicationDocument` shim deleted — gate cleared. ADR `docs/adr/0050-application-document-write-path-and-crm-ingest-safety.md` confirmed present. Push-shape match key confirmed scoped to `(crm_file_id, student_application_id)` in `UpsertCrmApplicationAction.php` (decided fix, validation Q1, landed).
- `app/Models` = **78 files**; `SHIMMED_MODELS = []`; `SHIMMED_MODEL_IMPORT_BASELINE` = 4 permanent entries (2 morph-backfill migrations + `FacilitiesDeliveryBoundaryArchTest.php` + `ClubMemberShimMorphBackfillMigrationTest.php` — one fewer than the plan's predicted 5-entry floor because the Phase 2 `EngagementQueryTicketModelPlacementArchTest` negative-reference assertion was deleted as redundant, not grandfathered). **Sweep 260811-0012 is COMPLETE.**
- `tests/Feature/Architecture` re-run 2026-08-16: 134 pass / 6 fail, same 6 pre-existing failures as Phase 1/2 checkpoints (2× `MigrationDebtInventoryTest` debt-ceiling drift, `CourseRosterDeliveryBoundaryArchTest`, `TeachingEligibilityAssignmentBoundaryTest`, `AcademicPeriodBoundaryArchTest`, `CourseDeliveryAssessmentBoundaryArchTest`) — none reference shim/DEAD surfaces; the 2 `MigrationDebtInventoryTest` failures are `shared_model_imports`/`legacy_filter_stacks` debt-ceiling drift from unrelated feature work landed after Phase 2, out of this plan's scope.
- Touched-module suites (Gold, Merchandise, Upload, Form, Admissions, StudentApplication, Api/V1/Student): 439 pass / 1 fail — same pre-existing `UploadPlatformTest` locale-message mismatch logged at Phase 2. Academic timeline test (run individually per known dir-abort flake): 7/7 pass.
- Remaining plan scope: Phase 10 (notification/email table drop) only — soak-gated, prod SSH + irreversible DROP, not started.

## Update 2026-08-16 (plan `260815-1320-close-remaining-legacy-shims-and-final-dead-cleanup`, Phase 10 — PLAN CLOSED)

User confirmed `notifications` + `email_templates` already dropped on prod. Drop migrations (`2026_08_09_160000_drop_notifications_table.php`, `2026_08_09_160100_drop_email_templates_table.php`) exist in repo since commit `1716254a5` (2026-08-09), each with a real `down()`. This session attempted independent SSH verification (`root@157.10.186.103`) but could not confirm the live prod target for `x.swin.edu.vn`: on-disk checkout stale (mtime 2025-08-06, no `vendor/`), no active nginx vhost found on that host, domain did not resolve via Cloudflare authoritative NS from this session's network. Backup/encryption/restore-rehearsal/FK-recheck/named-revert-range requirements are user-attested only, not evidence-verified this session.

**Plan `260815-1320-close-remaining-legacy-shims-and-final-dead-cleanup` CLOSED — all 10 phases complete.**
