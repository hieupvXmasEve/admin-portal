...
Charge and DNG pages: scope Selects always visible, preview on load/change, commit fields in a Dialog after primary click. After HP/EGC success, a no-query link to the DNG page.
...
- Functional: Primary «Sinh phí» / «Tạo lệnh thu» opens a Dialog (not an inline Card, not a step). Dialog holds DNG commit fields + ack; charge holds ack if >50. EGC block overrides stay on the table.
...
confirm Dialog (primary click)
  DNG: description, DatePicker due_date (portal-to dialog), estimate_time, ack
  Charge: ack if >50
...
<!-- Updated: user override — confirm Dialog on both pages -->
...
2. Commit: primary opens Dialog; `validateSetup` only for DNG Dialog submit; `wizard.commit()`.
3. DngPush: same layout. `validateSetup` only when submitting create Dialog. Fee type Select on inspect bar. Semester Select.
