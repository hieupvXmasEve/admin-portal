...
> **3a DONE (2026-08-01).** Backend + contract shipped, 56 test. RedemptionService (createOrder atomic, symmetric refund, 11 transitions), migrations (redemption_orders/items, stock_movements FK, gold_transactions partial-unique dedup key), student API (student.api.auth only — parent 403), staff API + RedemptionOrderPolicy, notification via Shared Contract, docs/api/student/merchandise.md, GoldTransactionController IDOR fix. Report: [cook-260801-1602-merchandise-store-phase-3a-redemption](../reports/cook-260801-1602-merchandise-store-phase-3a-redemption.md). **3b DONE (2026-08-02)** — student portal in FE/student-nuxt (store cards/detail, client-side cart, checkout pickup/shipping + Idempotency-Key + stale reconciliation, redemption history + timeline, gold history reusing useGoldWallet, student-only menu). typecheck + build green. Portal changes UNCOMMITTED in that repo's own git (user commits; 2 unrelated pre-existing untracked files there are not part of this). Contract shapes pinned (Swinx 8526464d). Report: [cook-260802-0049-merchandise-store-phase-3b-portal](../reports/cook-260802-0049-merchandise-store-phase-3b-portal.md). Deferred: staff cancellation-request notification (PII scoping = Notification-owner call); runtime smoke needs live backend.
