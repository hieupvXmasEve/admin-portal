...
| ✅ [3 — Redemption](phase-3-redemption.md) | **3a DONE 2026-08-01** (backend, 56 test, [report](../reports/cook-260801-1602-merchandise-store-phase-3a-redemption.md)) + **3b DONE 2026-08-02** (student portal FE/student-nuxt: store/cart/checkout/history/gold, typecheck+build green, [report](../reports/cook-260802-0049-merchandise-store-phase-3b-portal.md)). Portal changes UNCOMMITTED in its own repo (user commits). Deferred: staff cancellation-request notification (PII). | 1, 2 |
| ✅ [4 — Reports](phase-4-reports.md) | **DONE 2026-08-02** — read-only report queries (orders-by-status, gold used/refunded reconciled to ledger, most-redeemed, stock-by-campus) + admin Inertia UI + campus scope, 66 test. Excel export DEFERRED (stakeholder-gated). Also fixed a Phase-2 latent bug (abort_unless→500 on JSON). Report: [phase-4](../reports/cook-260802-0057-merchandise-store-phase-4-reports.md). | 2, 3 + stakeholder chốt |
