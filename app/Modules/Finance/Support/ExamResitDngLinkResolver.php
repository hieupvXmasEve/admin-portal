 ...
 * (ACAD-RET-002).
 *
 * A DNG request links to one or more FinanceCharges through the convenience
 * `finance_charge_id` and/or the `dng_payment_request_charges` pivot; an
 * exam-resit charge points back to its attempt via `ExamResitAttempt.finance_charge_id`.
...
class ExamResitDngLinkResolver
{
     ...
     * DNG request (directly or through the pivot).
     *
    ...
    public function chargeIdsWithActivePushedDng(array $chargeIds): array
    {
        ...
        return array_values(array_unique(array_map('intval', array_merge($direct, $pivot))));
    }
    ...
    private function chargeIdsForRequest(DngPaymentRequest $request): array
    {
        ...
        if ($request->finance_charge_id !== null) {
            $ids[] = (int) $request->finance_charge_id;
        }
    ...
    /**
     * @param  array<int>  $chargeIds
     * @return array<int, ExamResitAttempt> keyed by finance_charge_id
     */
