...
class DisposePaymentSurplusAction
{
    ...
    public function run(Payment $payment, array $data, int $approverId): PaymentSurplusDisposition
    {
        ...
        return DB::transaction(function () use ($payment, $data, $approverId): PaymentSurplusDisposition {
            ...
            if ($data['type'] === PaymentSurplusDisposition::TYPE_REALLOCATE) {
                ...
                $application = $this->settlement->createPaymentApplication(
                    $lockedPayment, $line, $amount, 'application', $approverId, self::class, null,
                );
            }
