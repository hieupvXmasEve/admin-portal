...
/**
 * Convert legacy egc_exempt_credit negative charge rows into FinanceCreditEntitlement
 * + credit applications (ADR-0030 / wave 5). Voids the negative line in the same
 * transaction. Hard-gates on unchanged remaining settlement.
 */
class BackfillLegacyEgcExemptCreditEntitlementsAction
{
    private const RECONCILE_TOLERANCE = 0.01;

    public const SOURCE_KIND = 'legacy_egc_exempt_credit';
    ...
    private function convertCharge(FinanceCharge $charge, array &$summary, bool $dryRun): void
    {
        ...
        $sourceRef = FinanceOwnedObligationSource::legacyEgcExemptCreditChargeRef((int) $charge->id);
        ...
        $this->voidFinanceChargeAction->handle(
            ...
            reason: 'Converted to FinanceCreditEntitlement (legacy egc_exempt_credit backfill)',
            userId: null,
        ...
        if ($mismatch !== null) {
            ...
            throw new RuntimeException(
                "EGC exempt credit conversion reconciliation failed for charge #{$charge->id}: {$mismatch}"
            );
        }
    ...
    private function detail(FinanceCharge $charge, string $status, string $reason, array $extra = []): array
    {
        return array_merge([
            ...
            'reason' => $reason,
            'source_kind' => self::SOURCE_KIND,
            'source_ref' => FinanceOwnedObligationSource::legacyEgcExemptCreditChargeRef((int) $charge->id),
        ], $extra);
    }
}
