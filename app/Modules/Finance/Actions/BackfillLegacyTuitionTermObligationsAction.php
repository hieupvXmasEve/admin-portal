...
/**
 * Backfill legacy tuition_term finance_charges into FinanceObligation rows.
 *
 * Does not create charges. Links existing active/voided tuition_term charges to
 ...
 */
class BackfillLegacyTuitionTermObligationsAction
{
    ...
    private function backfillCharge(FinanceCharge $charge, array &$summary, bool $dryRun): void
    {
        ...
        $sourceRef = FinanceOwnedObligationSource::legacyTuitionTermChargeRef((int) $charge->id);
        ...
        $obligation = FinanceObligation::query()->create([
            ...
            'currency' => 'VND',
            'pricing_rule_version' => 'tuition_term:legacy_backfill',
    ...
    /**
     * Hard-gate: derived settlement outstanding vs old charge balance.
     *
     * @param  array{
     *     checked:int,
     *     created:int,
    ...
    private function reconcileSettlement(
        ...
        if ($charge->status === FinanceCharge::STATUS_VOID) {
            // Voided payables are not collectable; record explicit pass for the hard-gate log.
        ...
        if (! $hasActiveLines) {
            // Pre-invoice: structural amount match only.
    ...
    /**
     * Dry-run hard-gate preview using invoice-line payable vs charge balance.
     *
     * @param  array{
     *     checked:int,
     *     created:int,
    ...
    /**
     * Old computed balance: charge.amount − paid − discount (active debits only).
     */
