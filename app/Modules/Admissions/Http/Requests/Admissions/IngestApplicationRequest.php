...
final class IngestApplicationRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            // ADR-0050: link is staff-facing (rendered as a raw href in the
            // admin UI) and CRM-supplied — restricted to http(s) so a
            // malicious/compromised payload cannot inject a javascript:/data:
            // URI that executes on click.
            'documents' => ['nullable', 'array'], 'documents.*.crm_file_id' => ['required', 'string', 'max:255'], 'documents.*.file_type_code' => ['required', 'string', 'max:255'], 'documents.*.file_type_name' => ['nullable', 'string', 'max:255'], 'documents.*.page_index' => ['nullable', 'integer', 'min:0'], 'documents.*.original_name' => ['nullable', 'string', 'max:255'], 'documents.*.link' => ['required', 'string', 'max:2048', 'regex:/^https?:\/\//i'], 'documents.*.mime_type' => ['nullable', 'string', 'max:100'], 'documents.*.size' => ['nullable', 'integer', 'min:0'], 'documents.*.status' => ['nullable', 'string', 'max:50'],
        ];
    }
}
