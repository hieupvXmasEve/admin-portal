...
final class EloquentStudentImpersonationTokenIssuer implements StudentImpersonationTokenIssuer
{
    public function issue(string $identifier, ImpersonationAdministrator $administrator): ImpersonationTokenResult
    {
        ...
        if (! $student->isActive()) {
            throw new \InvalidArgumentException('Cannot impersonate inactive student. Student status: '.$student->status);
        }
        ...
        return new ImpersonationTokenResult(['student' => ['id' => $student->id, 'student_id' => $student->student_id, 'user_id' => $student->user_id, 'full_name' => $student->full_name, 'email' => $student->email, 'status' => $student->status, 'campus' => $student->campus?->name, 'program' => $student->program?->name, 'specialization' => $student->specialization?->name, 'avatar_url' => $student->avatar_url], 'token' => $token, 'token_type' => 'Bearer', 'expires_at' => $expiresAt->toISOString(), 'impersonation_info' => ['impersonated_by' => ['id' => $administrator->id, 'name' => $administrator->name, 'email' => $administrator->email], 'impersonated_at' => now()->toISOString(), 'purpose' => $administrator->purpose ?? 'Admin support']]);
    }
}
