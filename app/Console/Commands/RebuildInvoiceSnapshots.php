 ...
 * ledger (finance_charges / invoice_lines / payments / discount_allocations).
 *
...
class RebuildInvoiceSnapshots extends Command
{
    ...
    public function handle(SettlementService $settlementService): int
    {
        ...
        $this->info("Rebuilding cached_* snapshot for {$total} invoice(s)...");
        ...
        $query->chunkById($chunkSize, function ($invoices) use ($settlementService, &$rebuilt, &$failed, $bar) {
        ...
        $bar->finish();
        $this->newLine(2);
        $this->info("Done. Rebuilt: {$rebuilt}. Failed: {$failed}.");
