    ...
    private function defineInvariants(): void
    {
        $this->invariants = [
            ...
            [
                'code' => 'INV-2',
                'severity' => 'HIGH',
                'label' => 'student_invoices.paid_amount cache drifts from live (active lines)',
                'count_sql' => 'SELECT COUNT(*) c FROM (
                    ...
                    GROUP BY si.id, si.paid_amount
                    HAVING ABS(si.paid_amount - COALESCE(SUM(pa.amount),0)) > 0.01
                ) t',
                'sample_sql' => 'SELECT si.id FROM student_invoices si
                    LEFT JOIN invoice_lines il ON il.invoice_id = si.id AND il.status = "active"
                    LEFT JOIN payment_applications pa ON pa.invoice_line_id = il.id
                    GROUP BY si.id, si.paid_amount
                    HAVING ABS(si.paid_amount - COALESCE(SUM(pa.amount),0)) > 0.01 LIMIT 5',
            ],
            [
                'code' => 'INV-3',
                'severity' => 'CRITICAL',
