...
use App\Modules\Finance\Actions\BackfillLegacyEgcExemptCreditEntitlementsAction;
use Illuminate\Console\Command;

class BackfillLegacyEgcExemptCreditEntitlementsCommand extends Command
{
    protected $signature = 'finance:backfill-legacy-egc-exempt-credit-entitlements
    ...
    protected $description = 'Convert legacy egc_exempt_credit negative charges into FinanceCreditEntitlement + credit applications';

    public function handle(BackfillLegacyEgcExemptCreditEntitlementsAction $action): int
    {
        ...
        if ($result['checked'] === 0) {
            $this->info('No active legacy egc_exempt_credit charges matched the conversion scope.');
        ...
        $this->info(sprintf(
            '%sChecked %d legacy egc_exempt_credit charge(s): converted %d, already converted %d, skipped %d, mismatch(es) %d.',
            $prefix,
            $result['checked'],
            $result['converted'],
