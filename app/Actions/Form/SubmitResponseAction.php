...
class SubmitResponseAction
{
    ...
    {
        $campus = Campus::find($data['campus_id'] ?? $student->campus_id);
        if (! $campus) {
            throw ValidationException::withMessages(['campus_id' => 'Campus not found.']);
        }

        $target = $this->findEligibleTarget($form, $student, $campus, $data);
        if (! $target) {
    ...
    private function findEligibleTarget(Form $form, Student $student, Campus $campus, array $data): ?FormTarget
    {
    ...
    private function dispatchQuerySubmittedNotification(QueryTicket $ticket, FormTarget $target, Student $student): void
    {
        ...
        $payload = $this->payloadBuilder->build('query_submitted', [
        ...
        $envelope = new DomainEventEnvelope(
            ...
            eventName: 'query.ticket_submitted',
            eventVersion: 1,
