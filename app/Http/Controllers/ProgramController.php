...
class ProgramController extends Controller
{
    ...
    {
        try {
            // Check if program has any specializations or curriculum versions
            if ($program->specializations()->count() > 0) {
                return back()->withErrors(['error' => 'Cannot delete program with existing specializations.']);
            }

            if ($program->curriculumVersions()->count() > 0) {
                return back()->withErrors(['error' => 'Cannot delete program with existing curriculum versions.']);
            }
            ...
            return back()->withErrors(['error' => 'Failed to delete program. Please try again.']);
        }
    }
