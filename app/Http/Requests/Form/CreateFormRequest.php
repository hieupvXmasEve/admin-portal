...
class CreateFormRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        $targets = collect($this->input('targets', []))
            ->map(function ($target) {
                if (array_key_exists('campus_id', $target)) {
                    $value = $target['campus_id'];

                    // Normalize "all"/empty/"0" to null to represent all campuses
                    if ($value === 'all' || $value === '' || $value === '0' || $value === 0) {
                        $target['campus_id'] = null;
                    } elseif (! is_null($value)) {
                        // Coerce numeric-looking values to int
                        $target['campus_id'] = is_numeric($value) ? (int) $value : $value;
                    }
                }
    ...
    public function rules(): array
    {
        return [
            ...
            'type' => ['required', 'string', Rule::in(['feedback', 'survey', 'query'])],
    ...
    {
        return [
            'code.required' => 'Form code is required',
            ...
            'targets.*.scope_type.required' => 'Target scope type is required',
            'targets.*.start_at.required' => 'Target start date is required',
        ];
    }
