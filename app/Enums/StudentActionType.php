...
enum StudentActionType: string
{
     ...
     *
     * The requires-decision set lives entirely on the status actions: dropout,
     * campus transfer, and the enrolment transitions into the study stages —
     * NE enrolment (→ `intake_pre_uni_gc`) and major enrolment (→ `intake_course`).
     * The matching EGC progression *events* (Placement Initialized, Course Stage
     * Changed) are a secondary record of those same transitions and never carry
     * the requirement. Academic defer and resume never require a Decision.
     *
     ...
     * in this set that still lack one. Admission deferral and waiting-for-course
     * records never require a Decision.
     */
    public function requiresDecision(): bool
    {
        return match ($this) {
            ...
            self::STUDENT_MAJOR_ENROLLMENT => true,
            default => false,
        };
    }
