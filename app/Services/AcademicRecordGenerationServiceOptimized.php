...
class AcademicRecordGenerationServiceOptimized
{
    ...
    /**
     * Sync academic records: Create new + Update existing in one operation (optimized for daily cron)
     * This combines generateAcademicRecords + updateAttendanceStatsForExistingRecords
     */
    public function syncAcademicRecords(?callable $progressCallback = null): array
    {
        $stats = [
            'created' => 0,
            'updated' => 0,
        ...
        try {
            // Get all student-course pairs from attendance data
            $studentCoursePairs = $this->getStudentCoursePairs();
            $total = $studentCoursePairs->count();
            $stats['total_processed'] = $total;

            if ($progressCallback) {
                $progressCallback(0, $total, 'Starting sync...');
            }
            ...
            foreach ($studentCoursePairs->chunk(self::CHUNK_SIZE) as $chunk) {
                DB::transaction(function () use ($chunk, &$stats) {
                    $this->processSyncChunk($chunk, $stats);
                });

                $processed += $chunk->count();
                if ($progressCallback) {
                    $progressCallback($processed, $total, "Synced {$processed}/{$total} records");
                }
            ...
            Log::error('Failed to sync academic records', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
            ]);
            throw $e;
    ...
    /**
     * Process sync chunk: upsert handles both create and update
     */
    private function processSyncChunk(Collection $chunk, array &$stats): void
    {
        // Pre-load data
        $studentIds = $chunk->pluck('student_id')->unique()->toArray();
        $courseOfferingIds = $chunk->pluck('course_offering_id')->unique()->toArray();
        ...
        $existingRecords = DB::table('academic_records')
            ->whereIn('student_id', $studentIds)
            ->whereIn('course_offering_id', $courseOfferingIds)
            ->select('student_id', 'course_offering_id')
            ->get()
            ->map(fn($r) => "{$r->student_id}_{$r->course_offering_id}")
            ->flip()
            ->toArray();
        ...
        foreach ($chunk as $pair) {
            ...
            $isExisting = isset($existingRecords[$key]);

            // Prepare data (upsert will handle both cases)
            $academicRecordsData[] = [
                'student_id' => $student->id,
                'course_offering_id' => $courseOffering->id,
                'semester_id' => $courseOffering->semester_id,
                ...
                'created_at' => now(),
                'updated_at' => now(),
                '_is_existing' => $isExisting, // For stats tracking
            ];
        ...
        if (!empty($academicRecordsData)) {
            try {
                // Count creates vs updates for stats
                foreach ($academicRecordsData as $record) {
                    if ($record['_is_existing']) {
                        $stats['updated']++;
                    } else {
                        $stats['created']++;
                    }
                ...
                // Remove tracking field before upsert
                $academicRecordsData = array_map(function ($record) {
                    unset($record['_is_existing']);
                    return $record;
                }, $academicRecordsData);

                DB::table('academic_records')->upsert(
                    $academicRecordsData,
                    ['student_id', 'course_offering_id'],
                    ['attendance_percentage', 'total_absences', 'total_present', 'total_late', 'total_not_recorded', 'total_class_sessions', 'meets_attendance_requirement', 'updated_at']
                );
            } catch (\Exception $e) {
                Log::error('Failed to upsert academic records', [
                    'error' => $e->getMessage(),
