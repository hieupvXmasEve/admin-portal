...
class EventNotificationService
{
    ...
    public function sendEventRegistrationConfirmation(Student $student, Event $event): void
    {
        $this->publish(
            ...
            deduplicationKey: "event:{$event->id}:student:{$student->id}:registration_confirmed",
            event: $event,
    ...
    public function sendEventCheckinConfirmation(Student $student, Event $event): void
    {
        ...
        $this->publish(
            ...
            deduplicationKey: "event:{$event->id}:student:{$student->id}:checkin_confirmed",
            event: $event,
            studentIds: [$student->id],
    ...
    {
        $this->publishGoldNotification($student, $amount, $event, false);
    }

    public function sendGoldReclaimNotification(Student $student, float $amount, Event $event): void
    {
        $this->publishGoldNotification($student, $amount, $event, true);
    }
        ...
        $event->participants()
            ...
            ->each(function (EventParticipant $participant) use ($event, $reason): void {
                ...
                $this->publish(
                    ...
                    event: $event,
                    studentIds: [$participant->student->id],
    ...
    public function sendEventCancellationConfirmation(Student $student, Event $event): void
    {
        $this->publish(
            ...
            deduplicationKey: "event:{$event->id}:student:{$student->id}:registration_cancelled",
            event: $event,
            studentIds: [$student->id],
            typeKey: 'event_registration',
    ...
    public function notifyEventUpdate(Event $event, Student $student): void
    {
        $this->publish(
            ...
            deduplicationKey: "event:{$event->id}:student:{$student->id}:updated:{$event->updated_at?->getTimestamp()}",
            event: $event,
            studentIds: [$student->id],
    ...
    private function publishGoldNotification(Student $student, float $amount, Event $event, bool $isReclaim): void
    {
        ...
        $this->publish(
            ...
            deduplicationKey: "event:{$event->id}:student:{$student->id}:gold:{$verb}:{$amount}",
            event: $event,
            studentIds: [$student->id],
